...
// md2pdf: upstream passes \hspace, \vspace and \raisebox arguments to `eval`,
// which would run document text as Typst code. Parse a plain length instead.
#let mitex-length(tex) = {
  let s = get-tex-str(tex)
  let m = if type(s) == str { s.trim().match(regex("^(-?(?:\\d+\\.?\\d*|\\.\\d+))\\s*(pt|mm|cm|in|em)$")) }
  if m == none { panic("unsupported length") }
  float(m.captures.at(0)) * (pt: 1pt, mm: 1mm, cm: 1cm, "in": 1in, em: 1em).at(m.captures.at(1))
}
...
#let (
  ...
  raisebox: define-cmd(2, handle: (sp, it) => text(baseline: -mitex-length(sp), it)),
  ...
  vspace: define-cmd(1, handle: it => v(mitex-length(it))),
  hspace: define-cmd(1, handle: it => h(mitex-length(it))),
