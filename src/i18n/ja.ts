...
const ja: Messages = {
  ...
  about: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          '中核にあるのは Typst。WebAssembly にコンパイルされ、ブラウザ上で動くモダンな組版システムです。改ページ、孤立行の制御、目次、ページ番号、脚注、PDF のしおりを処理し、コードのハイライトも内蔵のハイライタで行います。エンジンは約 7 MB。編集中にバックグラウンドで静かに読み込まれ、端末にキャッシュされるので、以降はオフラインでも使えます。',
        ],
      },
      {
    ...
    faq: [
      {
        ...
        answer: [
          'いいえ。解析・組版・PDF の生成はすべてブラウザ内で行われます。このサイトのコンテンツセキュリティポリシーは、サイト自身以外への通信を禁止しています。つまり「アップロードしない」は約束ではなく、ブラウザが強制する制約です。',
        ],
      },
      {
      ...
      },
      {
        ...
        answer: [
          'ページ自体は数十 KB で、すぐに開きます。組版エンジンは約 7 MB あり、ページの読み込み後にバックグラウンドで静かにダウンロードされます。たいていは書き終わる前に準備が整い、その状況は画面下のステータスバーに表示されます。データセーバー使用時や低速な回線では事前には取得せず、初めてダウンロードするときに読み込みます。エンジンは端末にキャッシュされるので、以降はオフラインでも変換できます。',
        ],
      },
      {
  ...
  ui: {
    ...
    engineWillLoad: 'PDF エンジンはバックグラウンドで読み込みます（約 7 MB）',
    ...
    engineSaveData: 'データセーバー：PDF エンジンはダウンロード時に読み込みます',
