...
const ko: Messages = {
  ...
  about: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: [
          '내부에서는 WebAssembly로 컴파일되어 브라우저에서 동작하는 최신 조판 시스템 Typst가 쓰입니다. 페이지 나눔, 외톨이 줄 방지, 목차, 쪽 번호, 각주, PDF 책갈피를 처리하고, 코드 강조도 내장 하이라이터가 맡습니다. 엔진 크기는 약 7 MB이며, 편집하는 동안 백그라운드에서 조용히 불러와 기기에 캐시되므로 그 뒤로는 오프라인에서도 쓸 수 있습니다.',
        ],
      },
      {
    ...
    faq: [
      ...
      },
      {
        ...
        answer: [
          '페이지 자체는 수십 KB라 바로 열립니다. 조판 엔진은 약 7 MB로, 페이지가 열린 뒤 백그라운드에서 조용히 내려받으며 보통 글을 다 쓰기 전에 준비가 끝납니다. 진행 상황은 하단 상태 표시줄에 나옵니다. 데이터 절약 모드나 느린 네트워크에서는 미리 받지 않고 처음 다운로드할 때 불러옵니다. 엔진은 기기에 캐시되므로 이후에는 오프라인에서도 변환할 수 있습니다.',
        ],
      },
      {
  ...
  ui: {
    ...
    engineWillLoad: 'PDF 엔진은 백그라운드에서 불러옵니다 (약 7 MB)',
    ...
    engineSaveData: '데이터 절약 모드: PDF 엔진은 다운로드할 때 불러옵니다',
